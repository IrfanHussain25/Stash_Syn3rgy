...
#     """
#     Sends image bytes to Google Cloud Vision API for label detection,
#     and returns a list of labels (tags).
#     """
#     try:
#         client = vision.ImageAnnotatorClient()
#         image = vision.Image(content=image_bytes) # Use bytes directly

#         print("Sending image bytes to Google Cloud Vision...")
...
#         if labels:
#             print(f"Cloud Vision detected labels: {labels}")
#             return labels
#         else:
#             print("Cloud Vision returned no high-confidence labels.")
#             return []

#     except Exception as e:
#         print(f"ERROR during Cloud Vision processing: {e}")
#         return None
...
# # # --- MODIFIED Background Task Function (handles image_url) ---
# # def process_item_async(item_id: str):
# #     print(f"[{datetime.utcnow()}] Background processing started for item_id: {item_id}")
# #     processed_data = None
# #     smart_stack = 'Other'
# #     final_status = 'failed'
...
# #         item_data = fetch_response.data[0]
# #         content_type = item_data.get('content_type')
# #         content = item_data.get('raw_content') # This is URL for web or image_url

# #         extracted_text = None # For URL/Text processing
# #         image_tags = None # For Image processing

# #         # 2. Process based on type
# #         if content_type == 'url' and content:
# #             print(f"Scraping URL: {content}")
# #             extracted_text = get_text_from_url(content)
# #         elif content_type == 'text' and content:
# #              extracted_text = content

# #         # --- NEW: Handle image_url ---
# #         elif content_type == 'image_url' and content:
# #             print(f"Processing image URL: {content}")
# #             try:
# #                 # Download image bytes from Supabase URL
# #                 image_response = requests.get(content, timeout=20)
# #                 image_response.raise_for_status()
# #                 image_bytes = image_response.content
# #                 print(f"Downloaded {len(image_bytes)} bytes from image URL.")

# #                 # Get tags using Cloud Vision
# #                 image_tags = get_tags_from_image_bytes(image_bytes)

# #                 if image_tags is not None:
# #                     # Format processed_data for images
# #                     processed_data = {
# #                         "title": f"Image - {image_tags[0]}" if image_tags else "Image",
# #                         "summary": f"Detected: {', '.join(image_tags[:5])}{'...' if len(image_tags) > 5 else ''}",
# #                         "primary_category": "Image",
# #                         "specific_tags": image_tags,
# #                         "key_info": {}
# #                     }
# #                     smart_stack = "Image"
# #                     final_status = 'processed'
# #                 else:
# #                     final_status = 'failed_vision' # Specific status
# #             except requests.exceptions.RequestException as req_e:
# #                 print(f"ERROR: Failed to download image from {content}: {req_e}")
# #                 final_status = 'failed_download'
# #             except Exception as vision_e:
# #                 print(f"ERROR during Cloud Vision processing for {item_id}: {vision_e}")
# #                 final_status = 'failed_vision'
# #         # --- END image_url Handling ---

# #         else:
# #              print(f"Skipping processing for content_type: {content_type}")
# #              final_status = 'skipped'

# #         # 3. Call Gemini (ONLY for URL/Text)
# #         if extracted_text and content_type in ['url', 'text']:
# #             print(f"Sending text ({len(extracted_text)} chars) to Gemini...")
# #             gemini_result = get_json_from_gemini(extracted_text)
# #             if gemini_result and isinstance(gemini_result, dict):
# #                 processed_data = gemini_result
# #                 smart_stack = gemini_result.get('primary_category', 'Other')
# #                 final_status = 'processed'
# #             else:
# #                  if final_status != 'failed': final_status = 'failed_gemini'
# #         elif content_type in ['url', 'text'] and not extracted_text:
# #             if final_status != 'failed': final_status = 'failed_extraction'
...
# #             update_payload = {'status': final_status, 'processed_data': processed_data, 'smart_stack': smart_stack}
# #             update_payload = {k: v for k, v in update_payload.items() if v is not None}
# #             if update_payload:
# #                  update_response = supabase.table('items').update(update_payload).eq('id', item_id).execute()
# #                  print(f"[{datetime.utcnow()}] DB update for {item_id}: Status={final_status}, Stack={smart_stack}.")
# #             else:
# #                  print(f"[{datetime.utcnow()}] No DB updates needed for {item_id}.")
# #         except Exception as db_e:
# #             print(f"ERROR: DB update failed for {item_id}: {db_e}")
...
#     """
#     Sends image bytes to Google Cloud Vision API for text detection (OCR)
#     and returns the extracted text.
#     """
#     try:
#         client = vision.ImageAnnotatorClient() # Client should be configured via GOOGLE_APPLICATION_CREDENTIALS
#         image = vision.Image(content=image_bytes)

#         print("Sending image to Google Cloud Vision for OCR (Text Detection)...")
#         # Use text_detection for dense text
#         response = client.text_detection(image=image)

#         if response.error.message:
#              raise Exception(f"Cloud Vision API (OCR) error: {response.error.message}")

#         if response.text_annotations:
#             # The first annotation usually contains the full detected text block
#             detected_text = response.text_annotations[0].description
#             # Check if significant text was found (more than just a few characters)
#             if detected_text and len(detected_text.strip()) > 10:
#                 print(f"Cloud Vision OCR successful, extracted {len(detected_text)} characters.")
#                 # Basic cleanup
#                 cleaned_text = '\n'.join(line.strip() for line in detected_text.splitlines() if line.strip())
#                 return cleaned_text[:15000] # Limit text size
#             else:
#                 print("Cloud Vision OCR found no significant text.")
#                 return None
#         else:
#             print("Cloud Vision OCR returned no text annotations.")
#             return None

#     except Exception as e:
#         print(f"ERROR during Cloud Vision OCR processing: {e}")
#         return None # Return None on failure

# # --- Background Task Function (REVISED for Unified Vision + Gemini on OCR) ---
# def process_item_async(item_id: str):
#     """
...
#         item_data = fetch_response.data[0]
#         content_type = item_data.get('content_type')
#         content = item_data.get('raw_content') # This is URL for web, image_url for images, or raw text
...
#              print(f"Skipping processing for item {item_id} with content_type: {content_type}")
#              final_status = 'skipped'
...
#              if not vision_tags: final_status = 'failed_extraction'
...
#                  processed_data_final.setdefault('title', 'Processing Issue') # Ensure defaults if Gemini output is partial
...
# # --- Endpoint: Capture Text/URL (Original, Unchanged) ---
# # This remains for handling non-image captures if needed, or could be merged
# @app.post("/v1/capture")
# async def capture_item(
#     item: CaptureItem,
...
#     user_email: str = Depends(get_current_user_email)
# ):
#     # Only allow 'url' or 'text' types here
...
#         item_id = insert_response.data[0]['id']
#         print(f"Item {item_id} (type: {item.type}) captured, pending.")
...
# # --- NEW Endpoint: Capture Image ---
# @app.post("/v1/capture/image")
# async def capture_image(
#     background_tasks: BackgroundTasks,
#     user_email: str = Depends(get_current_user_email),
#     file: UploadFile = File(...) # Use UploadFile for file input
# ):
#     """
#     Receives an image file, uploads to Supabase Storage, saves item
#     with 'image_url' type, and triggers background processing.
#     """
...
#         bucket_name = "stash-images" # Your public bucket name

#         print(f"Uploading image to Supabase Storage: {bucket_name}/{file_name}")

#         # Read file content
#         contents = await file.read()

#         # Upload to Supabase Storage
#         # Note: supabase-py storage API might change slightly, check its docs
#         storage_response = supabase.storage.from_(bucket_name).upload(file_name, contents, {"content-type": file.content_type})

#         # Check for upload errors (supabase-py might raise exception on failure)
#         # Assuming success if no exception is raised. Need to get URL.

#         # Get public URL
#         public_url_response = supabase.storage.from_(bucket_name).get_public_url(file_name)
#         image_url = public_url_response # The URL string

#         print(f"Image uploaded successfully. Public URL: {image_url}")

#         # Insert item into DB
#         insert_response = supabase.table('items').insert({
#             'content_type': 'image_url', # <-- New type
#             'raw_content': image_url,     # <-- Store the URL
#             'status': 'pending',
#             'user_email': user_email
#         }).execute()

#         if not insert_response.data or len(insert_response.data) == 0:
#              # Attempt to delete uploaded file if DB insert fails? (Optional cleanup)
#              try: supabase.storage.from_(bucket_name).remove([file_name])
#              except: pass
#              raise HTTPException(status_code=500, detail="Failed to save item metadata to database after upload.")

#         item_id = insert_response.data[0]['id']
#         print(f"Item {item_id} (type: image_url) captured, pending.")
...
#         raise http_exc
#     except Exception as e:
#         print(f"Error capturing image {item_id if item_id else '(unknown)'}: {e}")
#         # Add cleanup for storage if needed
#         raise HTTPException(status_code=500, detail=f"An unexpected error occurred during image capture: {str(e)}")
...
# --- Background Task Function (Handles all types) ---
# def process_item_async(item_id: str):
#     print(f"[{datetime.utcnow()}] Background processing started for item_id: {item_id}")
#     processed_data_final = None
#     smart_stack = 'Other'
...
#         # 1. Fetch item
#         fetch_response = supabase.table('items').select('id, raw_content, content_type').eq('id', item_id).limit(1).execute()
#         if not fetch_response.data: return
#         item_data = fetch_response.data[0]
#         content_type = item_data.get('content_type')
#         content = item_data.get('raw_content')

#         extracted_text_for_gemini = None

#         # 2. Process based on type
#         if content_type == 'url' and content:
#             extracted_text_for_gemini = get_text_from_url(content)
#             if not extracted_text_for_gemini: final_status = 'failed_extraction'

#         elif content_type == 'text' and content:
#              extracted_text_for_gemini = content

#         elif content_type == 'image_url' and content:
#             image_bytes = None
#             try:
#                 image_response = requests.get(content, timeout=25); image_response.raise_for_status()
#                 image_bytes = image_response.content
#                 print(f"Downloaded image bytes for {item_id}.")
#             except requests.exceptions.RequestException as req_e:
#                 print(f"ERROR: Failed image download {item_id}: {req_e}"); final_status = 'failed_download'

#             if image_bytes:
#                 try: # Vision Tagging
#                     temp_tags = get_tags_from_image_bytes(image_bytes)
#                     if temp_tags is not None:
#                         vision_tags = temp_tags; final_status = 'processed'; smart_stack = 'Image'
#                     else: print(f"Vision Tagging API call failed for {item_id}.")
#                 except Exception as vision_e: print(f"ERROR during Vision Tagging {item_id}: {vision_e}")

#                 try: # Vision OCR
#                     temp_ocr_text = get_text_from_image_bytes_gcv(image_bytes)
#                     if temp_ocr_text: extracted_text_for_gemini = temp_ocr_text
#                     else: print(f"Vision OCR found no text for {item_id}.")
#                 except Exception as ocr_e: print(f"ERROR during Vision OCR {item_id}: {ocr_e}")

#         else: final_status = 'skipped'

#         # 3. Call Gemini if text was extracted
#         if extracted_text_for_gemini:
#             gemini_result = get_json_from_gemini(extracted_text_for_gemini)
#             if gemini_result:
#                 gemini_analysis = gemini_result
#                 smart_stack = gemini_result.get('primary_category', smart_stack) # Prioritize Gemini's category
#                 final_status = 'processed' # Ensure processed if Gemini worked
#             else:
#                 final_status = 'failed_gemini' # Mark Gemini failure specifically
#         elif final_status == 'processed' and not vision_tags: # If vision tagging also failed (and no text)
#              final_status = 'failed' # Mark overall failure
#         elif final_status == 'pending': # If scraping failed or image had no text
#              if not vision_tags: final_status = 'failed_extraction' # Only fail if vision tags didn't work either


#         # 4. Combine Results
#         if final_status == 'processed':
#             processed_data_final = {}
#             if gemini_analysis: processed_data_final.update(gemini_analysis)
#             if vision_tags:
#                 processed_data_final['image_tags'] = vision_tags
#                 if not gemini_analysis: # Create structure if only vision worked
#                     processed_data_final.setdefault('title', f"Image - {vision_tags[0]}" if vision_tags else "Image")
#                     processed_data_final.setdefault('summary', f"Detected: {', '.join(vision_tags[:5])}{'...' if len(vision_tags) > 5 else ''}")
#                     processed_data_final.setdefault('primary_category', 'Image')
#                     processed_data_final.setdefault('specific_tags', vision_tags)
#                     processed_data_final.setdefault('key_info', {})
#             # Ensure defaults
#             processed_data_final.setdefault('title', 'Processed Item')
#             processed_data_final.setdefault('primary_category', smart_stack)
...
#             processed_data_final = None # Clear data on failure/skip

#     except Exception as e:
#         print(f"ERROR: Unhandled exception in background task {item_id}: {e}")
#         final_status = 'failed'; processed_data_final = None

# --- Background Task Function (MODIFIED Signature and Content Handling) ---
# --- Add 'original_content_for_processing' parameter ---
# def process_item_async(item_id: str, original_content_for_processing: str | None = None): 
#     """
#     Background task to process items.
#     Uses 'original_content_for_processing' for scraping/analysis if provided.
#     """
#     print(f"[{datetime.utcnow()}] Background processing started for item_id: {item_id}")
#     processed_data_final = None
#     smart_stack = 'Other'
...
#     try:
#         # 1. Fetch item type from DB (still need this)
#         fetch_response = supabase.table('items').select('id, content_type').eq('id', item_id).limit(1).execute()
#         if not fetch_response.data:
#             print(f"ERROR: Item {item_id} not found.")
#             return
#         item_data = fetch_response.data[0]
...
#         # --- FIX: Use passed content directly for processing ---
#         content_to_process = original_content_for_processing
#         # ----------------------------------------------------

#         extracted_text_for_gemini = None

#         # 2. Process based on type using 'content_to_process'
#         if content_type == 'url' and content_to_process:
#             print(f"Scraping ORIGINAL URL: {content_to_process}")
#             extracted_text_for_gemini = get_text_from_url(content_to_process) # Scrape original
#             if not extracted_text_for_gemini: final_status = 'failed_extraction'

#         elif content_type == 'text' and content_to_process:
#              extracted_text_for_gemini = content_to_process # Use original text
#              print(f"Using raw text content for item {item_id}.")

#         elif content_type == 'image_url' and content_to_process: # Image URL from Supabase (already clean)
#             print(f"Processing image URL: {content_to_process}")
#             image_bytes = None
#             try: # Download image
#                 image_response = requests.get(content_to_process, timeout=25); image_response.raise_for_status()
#                 image_bytes = image_response.content
#             except requests.exceptions.RequestException as req_e:
#                 print(f"ERROR: Failed image download {item_id}: {req_e}"); final_status = 'failed_download'

#             if image_bytes:
#                 # Run Vision Tagging & OCR (using image_bytes)
#                 # ... (Vision Tagging and OCR logic remains the same) ...
#                  try: # Vision Tagging
#                     temp_tags = get_tags_from_image_bytes(image_bytes)
#                     if temp_tags is not None: vision_tags = temp_tags; final_status = 'processed'; smart_stack = 'Image'
#                     else: print(f"Vision Tagging API call failed for {item_id}.")
#                  except Exception as vision_e: print(f"ERROR during Vision Tagging {item_id}: {vision_e}")

#                  try: # Vision OCR
#                     temp_ocr_text = get_text_from_image_bytes_gcv(image_bytes)
#                     if temp_ocr_text: extracted_text_for_gemini = temp_ocr_text # Get text for Gemini
#                     else: print(f"Vision OCR found no text for {item_id}.")
#                  except Exception as ocr_e: print(f"ERROR during Vision OCR {item_id}: {ocr_e}")

#         else: final_status = 'skipped'

#         # 3. Call Gemini (If text extracted)
#         # ... (Gemini call logic remains the same) ...
#         if extracted_text_for_gemini:
#             gemini_result = get_json_from_gemini(extracted_text_for_gemini)
#             if gemini_result:
#                 gemini_analysis = gemini_result
#                 smart_stack = gemini_result.get('primary_category', smart_stack)
#                 final_status = 'processed'
#             else: final_status = 'failed_gemini'
#         elif final_status == 'processed' and not vision_tags: final_status = 'failed'
#         elif final_status == 'pending':
#              if not vision_tags: final_status = 'failed_extraction'


#         # 4. Combine Results (Unchanged)
#         # ... (Logic to combine gemini_analysis and vision_tags remains the same) ...
#         if final_status == 'processed':
#             processed_data_final = {}
#             if gemini_analysis: processed_data_final.update(gemini_analysis)
#             if vision_tags:
#                 processed_data_final['image_tags'] = vision_tags
#                 if not gemini_analysis: # If only vision worked
#                     processed_data_final.setdefault('title', f"Image - {vision_tags[0]}" if vision_tags else "Image")
#                     processed_data_final.setdefault('summary', f"Detected: {', '.join(vision_tags[:5])}{'...' if len(vision_tags) > 5 else ''}")
#                     processed_data_final.setdefault('primary_category', 'Image')
#                     processed_data_final.setdefault('specific_tags', vision_tags)
...
#             processed_data_final.setdefault('primary_category', smart_stack)
#             processed_data_final.setdefault('specific_tags', [])
#         elif final_status != 'pending': processed_data_final = None
...
#         # 5. Update DB
#         try:
#             update_payload = {'status': final_status, 'processed_data': processed_data_final, 'smart_stack': smart_stack}
#             update_payload = {k: v for k, v in update_payload.items() if k == 'processed_data' or v is not None}
#             if update_payload.get('status') != 'pending' or 'processed_data' in update_payload:
#                 supabase.table('items').update(update_payload).eq('id', item_id).execute()
#                 print(f"[{datetime.utcnow()}] DB update for {item_id}: Status={final_status}, Stack={smart_stack}.")
#             else: print(f"[{datetime.utcnow()}] No final DB updates needed for {item_id}.")
#         except Exception as db_e: print(f"ERROR: DB update failed critically for {item_id}: {db_e}")
...
#     try:
#         insert_response = supabase.table('items').insert({
#             'content_type': item.type, 'raw_content': item.content,
...
#         item_id = insert_response.data[0]['id']
#         background_tasks.add_task(process_item_async, item_id)
#         print(f"Item {item_id} ({item.type}) captured, task added.")
#         return {"status": "success", "data": insert_response.data[0]}
#     except Exception as e:
#          print(f"Error capturing {item.type}: {e}")
#          if isinstance(e, HTTPException): raise e
...
# --- NEW Endpoint: AI-assisted Keyword Search ---
# --- Endpoint: AI-assisted Keyword Search (REVISED FILTER) ---
# --- Endpoint: AI-assisted Keyword Search (REVISED FILTER v2) ---
# --- Endpoint: AI-assisted Keyword Search (SIMPLE FILTER) ---
# --- Endpoint: AI-assisted Keyword Search (USING RPC) ---
